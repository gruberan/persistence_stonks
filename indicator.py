...
def date_by_subtracting_exchange_calendar_days(from_dt, num_days, exchange='NYSE'):
    ...
    Arguments:
        ...
        exchange (str): which stock exchange to go by
    """
...
def compute_window_landscapes(start_dt, end_dt, ticker_symbol_list, window_size, maxdim=1):
    """
        ...
        start_dt (str or datetime.datetime): first date to compute the landscapes for (note that window_size calendar
        dates must be retrievable before this date) start_dt (str or datetime.datetime): last date to compute the
        landscapes for maxdim (int): Maximum homology dimension computed. Will compute all dimensions lower than and
        equal to this value. For 1, H_0 and H_1 will be computed.
    """
...
def plot_window_landscapes(start_dt, end_dt, ticker_symbol_list=[], window_size=None, row_syms=["^GSPC", "L0", "L1"],
    ...
    if use_precomputed_df:
        df = precomputed_df.drop(
            precomputed_df.index[(precomputed_df.index < start_dt) | (precomputed_df.index > end_dt)])
    else:
        df = compute_window_landscapes(start_dt=start_dt, end_dt=end_dt, ticker_symbol_list=ticker_symbol_list,
